...
class Sentence2Graph(object):
    ...
    def __init__(self, args, kg_folder, num_nodes=None):
        """
        ...
        self.all_x = torch.load(osp.join(kg_folder, 'wnrr_x.pt'))
        all_edge_index = torch.load(osp.join(kg_folder, 'wnrr_edge_index.pt'))
        all_edge_type = torch.load(osp.join(kg_folder, 'wnrr_edge_type.pt'))
        all_edge_attr = torch.load(osp.join(kg_folder, 'wnrr_edge_attr.pt'))
        knn_edge_index = torch.load(osp.join(kg_folder, 'wnrr_knn_edge_index.pt'))
        ...
        if args.knn:
            ...
            self.all_edge_type = all_edge_type
            self.all_edge_attr = torch.cat([all_edge_attr, all_edge_attr.mean(dim=0).unsqueeze(dim=0)], dim=0)
    ...
    def reduce(self, batched_sentence, raw=False):
        """
        ...
        edge_type = edge_type.long().cuda()
        edge_attr = edge_attr.cuda()
        num_nodes = num_nodes.cuda()
        num_edges = num_edges.cuda()
        
        flat_entities_id = []
        for entities in entities_id:
            flat_entities_id.extend(entities)
        flat_entities_id = torch.tensor(flat_entities_id).long().cuda()
    ...
    def reduce_connected(self, batched_sentence, raw=False):
        """
        ...
        flat_entities_id = []
        x = torch.tensor([]).cuda()
        edge_index = torch.tensor([[], []]).long().cuda()
        edge_attr = torch.tensor([]).cuda()
        edge_type = torch.tensor([]).long().cuda()
        for subset in entities_id:
            ...
            if n < 2:
                ...
                x = torch.cat([x, self.all_x[subset].cuda()], dim=0)
            ...
            sub_nodes = torch.tensor(sub_nodes).long().unique().cuda()
            num_nodes.append(sub_nodes.size(0))
            return_x = self.all_x[sub_nodes].cuda()
            
            return_edge_index = torch.tensor(return_edge_index).long().T.cuda()
            return_edge_index = self.__relabel__(sub_nodes, return_edge_index) + cum_nodes[-1]
            
            return_edge_type = torch.tensor(return_edge_type).cuda()
            return_edge_attr = self.all_edge_attr[return_edge_type].cuda()
        ...
        num_edges = num_edges.cuda()
        flat_entities_id = torch.tensor(flat_entities_id).long().cuda()
        batch = torch.tensor([i for i in range(len(num_nodes)) for _ in range(num_nodes[i])]).long().cuda()
