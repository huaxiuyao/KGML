...
# code from leaf: https://github.com/TalwalkarLab/leaf
import re
import numpy as np
import json
...
def line_to_indices(line, tokenizer, max_words=10):
    ...
    indl = np.array([tokenizer.convert_tokens_to_ids(w) for w in line_list])
    mask = np.array(indl == 1, dtype=np.bool) # unrecognized
    indl = list(indl[~mask][:max_words])
    indl = list(indl[:max_words])
    mask = np.zeros(max_words)
    mask[: len(indl)] = 1
    indl += [0]*(max_words-len(indl))
    return indl, mask
