...
def evaluate_groups(args, model, loader, epoch=None, split='val', n_samples_per_dist=None):
    ...
    for group in loader.dataset.groups:
        dist_id = group
        ...
        for i, idx in enumerate(example_ids):
            x, y, g, mask, raw_x, raw_y = loader.dataset[idx]
            X.append(x); Y.append(y); G.append(g); M.append(mask); Raw.append(raw_x)
            if (i + 1) % args.support_size == 0:
                X, G, M = torch.stack(X), torch.tensor(G, dtype=torch.long), torch.stack(M)
                
                if args.dataset == 'reddit':
                    Y = torch.stack(Y).long()
                elif args.dataset == 'twitter':
                    Y = torch.tensor(Y, dtype=torch.long)
                batches.append((X, Y, G, M, Raw))
                X, Y, G, M, Raw = [], [], [], [], []
        ...
        counter = 0
        for x, labels, group_id, mask, raw_x in batches:
            labels = labels.detach().numpy()
            x = x.to(args.device)
            mask = mask.to(args.device)
            logits = model(x, mask, raw_x)
        ...
        # Evaluate
        if args.dataset == 'twitter':
            accuracy = np.mean(preds_all == labels_all.reshape(-1))
        elif args.dataset == 'reddit':
...
def eval_dist(dist, model, loader, args, n_samples_per_dist):
    ...
    counter = 0
    for x, labels, group_id, _, _ in loader:
