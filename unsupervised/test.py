...
def test(args, eval_on):
    ...
    # Get data
    train_loader, train_eval_loader, val_loader, test_loader = datasets.get_loaders(args)
    ...
    model = models.ContextualNet(
        ContextNet=models.RNNContextModel, PredictionNet=Dict['PredictionNet'],  # context model and predictor
        ...
        context_hidden_dim=Dict['context_hidden_dim'], context_output_dim=Dict['context_output_dim'],  # dims for context model
        use_context=args.use_context, rnn_type=Dict['rnn_type'],  # specify whether to use context net, and the type for the context net(if YES)
        support_size=args.support_size,  # meta-learning
        from_pretrain=Dict['from_pretrain'], ntoken=Dict['ntoken'], embed_dir=Dict['embed_dir'],  # whether to use pretrained embed
        seq_len=seq_len, use_last=False
    ).double()
